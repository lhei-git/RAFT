...
const Results = ({ getData }) => {
  ...
  useEffect(() => {
    if (inputs.cluster.length > 0 && Object.keys(inputs.training_data).length > 0) {
    setTrainingPlotData([
      {
  ...
  const generateClusterTable = (temps) => {
    ...
    let tempCluster = ['Low Temp Cluster', 'Mid Temp Cluster', 'High Temp Cluster']
    ...
    return tempCluster.map((t, i) => 
      <tr key={i}>
        <td style={{ fontWeight: 700 }}>{tempTitle[i]}</td>
        <td>
        ...
        {/* <td>
          {average(...temps[tempCluster2[i]]).toFixed(2)}
          &#176;C
        </td>
        <td>
          {average(...temps[tempCluster3[i]]).toFixed(2)}
          &#176;C
        </td> */}
      </tr>
    )
    ...
    return (
      <>
        <Table responsive="sm">
          ...
          <tbody>
              {Object.keys(inputs.model).length > 0 ? generateModelDataTable(inputs.model) : generateSkeleton(3,4)}
          </tbody>
        </Table>
      </>
    );
    ...
    return (
      <>
        <Table responsive="sm">
          ...
          <tbody>
              {inputs.cluster.length > 0 ? generateClusterTable(inputs.cluster[0]) : generateSkeleton(3,6)}
          </tbody>
        </Table>
      </>
    );
  ...
  const getPlot = useCallback((data, layout) => {
      ...
      <Table responsive="sm">
        ...
        <div className="plots">
            <div className="histTempDataPlot">
              {trainingPlotData.length > 0 ? getPlot(trainingPlotData, {title: 'Historical Temperature Data'}) : <Skeleton variant="rect" width={500} height={500} />}
            </div>
            <div className="histRangeTempsPlot">
              {clusterPlotData.length > 0  ? getPlot(clusterPlotData, {title: 'Clustered Historical Temperatures',}) : <Skeleton variant="rect" width={500} height={500} />}
            </div>
          </div> 
      </>
    </div>
